...
my $keypair_seed = join('',
    map { chr($_) } 0x42, 0x11, 0x51, 0xa4,
    0x59, 0xfa, 0xea, 0xde,
    0x3d, 0x24, 0x71, 0x15,
    0xf9, 0x4a, 0xed, 0xae,
    0x42, 0x31, 0x81, 0x24,
...
for ($i = 0; $i < scalar @tests; $i++) {
    ...
    my $skpk    = substr($sk_bin, 0, $crypto_sign->SEEDBYTES)
        . substr($pk_bin, 0, $crypto_sign->PUBLICKEYBYTES);
    my $msg = $test->{msg};
    $msg_bin = hex2bin($msg);
    ...
    is(bin2hex(substr($s_sealed, 0, $crypto_sign->BYTES)),
        $test->{sig}, "signature $t correct");
    ...
    eval { my $mod_opened = $crypto_sign->open($mod_sealed, $pk_bin); };
...
for (my $j = 1; $j < 8; $j++) {
    ...
    ok(
        !$crypto_sign->verify($s_mac, $msg_bin, $pk_bin),
...
ok(
    !$crypto_sign->verify(
        $s_mac, $msg_bin, "\0" x $crypto_sign->PUBLICKEYBYTES
    ),
...
sub getTestData {
    ...
    while (my $line = <$fh>) {
        ...
        push @tests, {sk => $sk, pk => $pk, sig => $sig, msg => $msg,};
    }
