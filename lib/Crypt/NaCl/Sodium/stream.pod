...
Crypt::NaCl::Sodium::stream - Stream Ciphers
...
xsalsa20
    ...
    my $random_bytes = $crypto_stream->bytes(256, $nonce, $key);
...
chaha20
    ...
    # generate 256 bytes from $nonce and $key
    my $random_bytes = $crypto_stream->chacha20_bytes(256, $nonce, $key);
...
Salsa20
    ...
    # generate 256 bytes from $nonce and $key
    my $random_bytes = $crypto_stream->salsa20_bytes(256, $nonce, $key);
...
The L<crypto_stream|https://doc.libsodium.org/advanced/stream_ciphers>
provides C<Salsa20>, C<XSalsa20>, C<ChaCha20>, and C<XChacha20> stream
ciphers which can be used to generate pseudo-random data from a C<key>
and C<nonce>, or as building blocks for implementing custom
constructions, but they are not alternatives to
L<Crypt::NaCl::Sodium::secretbox>.

Nonce (number used once) does not have to be protected, but it is
crucial that the same nonce is never reused with the same key.
